 ...
 *  - Each time the tag is re-tapped, a slightly changed TEXT record and MIME record can be read out.
 *  - When the WAKEUP button is pressed, the NFC memory will be populated with an single-record NDEF message containing
 ...
 *      an NDEF message, its contents are immediately parsed. Any TEXT record's payload is copied and used thereafter in
 *      subsequent messages generated by the tag; as is the case for any MIME record's payload.
 *      Resetting the IC will revert back to the original payload set.
 *
 * @par Application flow
 *  Once powered, the application will continue run until the power source is removed externally.
 ...
 * @par Tag reader
 *  - on Android:
 *      - Any NFC-enabled Android phone can be used to read out text and URLs generated by the tag, even without any app
 *          installed or active.
 ...
 *      - The iOS App Store contains multiple NFC enabled apps that allow to write NDEF messages.
 *  - on Windows and macOS:
 *      - The <em>NTAG SmartSensor Automator</em> application, available in the SDK under <em>sw/QT/comm</em>,
